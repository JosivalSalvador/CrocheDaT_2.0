...
Este repositório centraliza o código-fonte da plataforma CrocheDaT, estruturada como um monorepo que agrega o backend (API), frontend (Web) e configurações de infraestrutura. A arquitetura visa a separação de responsabilidades mantendo um fluxo de desenvolvimento unificado via Docker e Scripts NPM. (top)
