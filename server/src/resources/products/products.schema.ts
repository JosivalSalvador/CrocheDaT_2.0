...
export const createProductSchema = z.object({
  ...
  productionTime: z.number().int().positive({ message: 'O tempo de produção deve ser um número inteiro positivo' }),

  price: z.number().positive({ message: 'O preço deve ser um valor positivo' }),

  categoryId: z.uuid({ message: 'ID de categoria inválido' }),
...
// ==========================================
export const updateProductSchema = createProductSchema.omit({ images: true }).partial()
...
export const imageIdParamSchema = z.object({
  imageId: z.uuid({ message: 'ID da imagem inválido' }),
})
